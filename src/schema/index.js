...
function createSchema({pages = []}) {
      ...
      if (isBuiltInScalarType(type)) {
        ...
        filterMethods.forEach((filterMethod) => {
          PagesWhereInputITC.addFields({
            [`${prefix ? `${prefix}${DEVIDER_OBJECT}` : ''}${k}${DEVIDER_METHOD}${filterMethod}`]: type,
          })
        })
        // Add sort methods
        sortMethods.forEach((sortMethod) => {
          PagesOrderByInputETC.addFields({
            [`${prefix ? `${prefix}_` : ''}${k}_${sortMethod}`]: { value: sortFunctions.length }
          })
          sortFunctions.push([
            [`${prefix ? `${prefix}.` : ''}${k}`], {reverse: sortMethod === sortMethods[1]}
          ])
        })
      }
    })
    ...
    resolve: ({args: {skip, limit, orderBy, where}}) => {
      ...
      if (where) {
        Object.keys(where).forEach((k) => {
          ...
          filterFns.push(FILTER_FN[method](keyName.replace(DEVIDER_OBJECT, '.'), where[k]))
        })
      }
  ...
  PageTC.addResolver({
    ...
    resolve: ({args: {id, slug}}) => {
      const fn = slug ? (item) => item['slug'] === slug : (item) => item['id'] === id
  ...
  PageTC.addResolver({
    ...
    resolve: ({args: {id}}) => {
      const findParent = (item, list = []) => {
        if (item.parentId) {
          return findParent(pages.find(page => page.id === item.parentId), [item, ...list])
        }
        return [item, ...list]
      }
